...
results <- readRDS("final-results.rds")
...
Avg_res_k0 = list(Avg_truth_W_tate = Avg_truth_W_tate_k0, 
                  Avg_bias = Avg_bias_k0, Avg_MAE = Avg_MAE_k0, 
...
Avg_MAE_k0.05 = colMeans(abs(Bias))
Avg_MAE_k0.05
...
Avg_res_k0.05 = list(Avg_truth_W_tate = Avg_truth_W_tate_k0.05, 
                     Avg_bias = Avg_bias_k0.05, Avg_MAE = Avg_MAE_k0.05, 
...
Avg_MAE_k0.2 = colMeans(abs(Bias))
Avg_MAE_k0.2
...
Avg_res_k0.2 = list(Avg_truth_W_tate = Avg_truth_W_tate_k0.2, 
                    Avg_bias = Avg_bias_k0.2, Avg_MAE = Avg_MAE_k0.2, 
...
Avg_MAE_k0.5 = colMeans(abs(Bias))
Avg_MAE_k0.5
...
Avg_res_k0.5 = list(Avg_truth_W_tate = Avg_truth_W_tate_k0.5, 
                    Avg_bias = Avg_bias_k0.5, Avg_MAE = Avg_MAE_k0.5, 
...
Avg_MAE_k0.8 = colMeans(abs(Bias))
Avg_MAE_k0.8
...
Avg_res_k0.8 = list(Avg_truth_W_tate = Avg_truth_W_tate_k0.8, 
                    Avg_bias = Avg_bias_k0.8, Avg_MAE = Avg_MAE_k0.8, 
...
res2vec = function(avg_res){
  dt = matrix(c(as.numeric(avg_res$Avg_truth_W_tate[-1]),
                avg_res$Avg_bias,avg_res$Avg_MAE, avg_res$Avg_cov,
...
colnames(avg_tb) = c("Truth","ATE_OR","bias_OR","MAE_OR","cov_OR",
                     "CI_length_OR","ATE_IOW","bias_IOW","MAE_IOW",
                     "cov_IOW","CI_length_IOW","ATE_IOW2","bias_IOW2",
                     "MAE_IOW2","cov_IOW2","CI_length_IOW2","ATE_DRE1",
                     "bias_DRE1","MAE_DRE1","cov_DRE1","CI_length_DRE1",
                     "ATE_DRE2","bias_DRE2","MAE_DRE2","cov_DRE2",
                     "CI_length_DRE2")
...
colnames(avg_tb2) = c("Truth","ATE_OR","bias_OR","MAE_OR","ATE_IOW",
                      "bias_IOW","MAE_IOW","ATE_IOW2","bias_IOW2",
                      "MAE_IOW2","ATE_DRE1","bias_DRE1",
                      "MAE_DRE1","ATE_DRE2","bias_DRE2","MAE_DRE2")
...
MAE_sub = avg_tb %>% select(MAE_OR,MAE_IOW,MAE_IOW2,
                                 MAE_DRE1,MAE_DRE2)
print(xtable(MAE_sub, type = "latex"))
...
## ATE 
method = rep(c("Truth","OR" ,"IOW1" ,"IOW2" ,"DRE1" ,"DRE2"),each = length(k_lst))
k = rep(k_lst,6)
...
## Bias 
method = rep(c("OR" ,"IOW1" ,"IOW2" ,"DRE1" ,"DRE2"),each = length(k_lst))
k = rep(k_lst,5)
...
method = rep(c("OR" ,"IOW1" ,"IOW2" ,"DRE1" ,"DRE2"),each = length(k_lst))
k = rep(k_lst,5)
MAE_lst = as.numeric(unlist(MAE_sub))
MAE_tb = cbind.data.frame(Method = method,K = k,MAE = MAE_lst)

ggplot(data=MAE_tb, aes(x=K, y=MAE_lst, group=Method)) +
  geom_line(aes(color=Method))+
  geom_point(aes(color=Method))+ 
  xlab("K") +
  ylab("MAE") +
  ggtitle("Mean Absolute Error under different Method and different K")
...
create_long = function(result){
  Method = rep(c("OR" ,"IOW1" ,"IOW2" ,"DRE1" ,"DRE2"),
  ...
  long_tb$Method = factor(long_tb$Method, 
                          levels = c("OR" ,"IOW1" ,"IOW2","DRE1" ,"DRE2"))
  return(long_tb)
}
